...
To provide a comprehensive, hands-on demonstration of how to integrate and use the `typo3fluid-storybook-js-integration` tool in a functional TYPO3 environment, a dedicated example project and repository is planned.

This example aims to showcase best practices and cover key features such as rendering various Fluid templates, using the template discovery script, and fetching dynamic data from TYPO3 into Storybook stories.

### Intended Structure and Features

The planned example project will include:

*   A minimal TYPO3 (e.g., v12) setup.
*   A basic TYPO3 extension (`example_integration`) containing:
    *   Several Fluid templates (Templates, Partials, Layouts).
    *   A PHP `ApiController` (or similar) providing:
        *   A `/fluid/render` API endpoint for rendering Fluid templates via `StandaloneView`.
        *   A simple `/api/data/content_element/{uid}` endpoint for serving dynamic JSON data from TYPO3 (e.g., from `tt_content` records).
*   A `storybook_setup/` directory (or similar, at the root of the example project) containing:
    *   A full Storybook configuration (`main.ts`, `preview.ts`, etc.).
    *   `package.json` with Storybook dependencies and scripts to run Storybook and the `discover-fluid-templates.js` script (copied or linked from this main package).
    *   Example Storybook stories (`.stories.ts`) demonstrating:
        *   Rendering simple Fluid templates with static variables.
        *   Using Storybook `loaders` to fetch and render dynamic data from the example TYPO3 API.
        *   Utilizing the `fluid-templates.json` generated by the discovery script for template selection in stories.
*   A root `README.md` with detailed setup instructions for the example project itself, including TYPO3 setup, API endpoint creation, and Storybook configuration.

### Status & How to Access

*   **Status:** The dedicated example repository is currently `[Planned]`.
*   Contributions towards creating and maintaining this example project are welcome! If you are interested, please open an issue in this repository to discuss.
*   (Once available, a link to the repository will be provided here: `[Link to Example Repository - To Be Created]`)

### High-Level Setup Outline (Conceptual for the Example Project)

Once available, setting up and running the example project would typically involve these steps:

1.  **Clone Repository:** Clone the dedicated example project repository.
2.  **TYPO3 Setup:**
    *   Set up a local TYPO3 instance (e.g., using DDEV, Docker, or a local web server like XAMPP/MAMP).
    *   Install the specified TYPO3 version (e.g., v12).
    *   Run `composer install` in the example project's TYPO3 root to get necessary PHP dependencies.
    *   Activate the `example_integration` TYPO3 extension via the TYPO3 backend or command line.
    *   Ensure the API routes provided by `example_integration` are accessible.
3.  **Storybook Setup:**
    *   Navigate to the `storybook_setup/` directory within the example project.
    *   Run `npm install` (or `yarn install`) to get Storybook and other Node.js dependencies.
4.  **Configuration:**
    *   In the `storybook_setup/` directory, create or update the `.env` file.
    *   Set `TYPO3FLUID_STORYBOOK_API_URL` to point to your local TYPO3's `/fluid/render` endpoint.
    *   Set `STORYBOOK_TYPO3_DATA_API_BASE_URL` to point to your local TYPO3's data API base URL (e.g., for `/api/data`).
5.  **Discover Templates:**
    *   Run the template discovery script (e.g., via an `npm run discover-templates` script defined in the example's `package.json`) to generate the `fluid-templates.json` file. This script would target the `example_integration` extension path.
6.  **Run Storybook:**
    *   Execute `npm run storybook` (or `yarn storybook`) from the `storybook_setup/` directory to start the Storybook development server and view the integrated Fluid templates.

This example project will serve as a valuable resource for users looking to implement this integration in their own TYPO3 projects.
